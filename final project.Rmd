...
summary(data)
```
## remove 60 and 200 outliers on ap_hi
## remove __ and __ outliters on ap_lo
## remove 140 and 190 out height
## remove weight
...
```{r}
library(randomForest)
rf.cardio_reduced <- randomForest(cardio ~ ap_hi + cholesterol + age + ap_lo + gender
                                  , data = selected, subset = train_indices,
...
```{r}
yhat.boost <- predict(boost.carido, newdata = data[-train_indices,], n.trees = 1000)
